...
func _on_area_2d_body_entered(body:Node2D):
	...
	#var duplicate_skin = body.duplicate(true)
	
	#votex.add_child(duplicate_skin)
	votex.call_deferred("add_child", duplicate_skin)
	
	print()
	#print("body position", body.position)
	#print("body global_position", body.global_position)
	#print("clone position", duplicate_skin.position)
	#print("clone global_position", duplicate_skin.global_position)
	
	#duplicate_skin.position = body.position - self.position
	print()
	#print("clone position", duplicate_skin.position)
	#print("clone global_position", duplicate_skin.global_position)
	...
	#_tween.tween_callback(duplicate_skin).set_delay(delay_time)
...
func _on_timer_reset_votex_timeout():
	#for i in votex.get_children():
		#i.queue_free()
	pass # Replace with function body.
